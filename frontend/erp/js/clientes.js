...
function abrirEdicaoClientePendente() {
    const pendingId = sessionStorage.getItem('cds-erp-cliente-id');
    if (!pendingId) return;
    sessionStorage.removeItem('cds-erp-cliente-id');
    if (typeof editCliente === 'function') {
        editCliente(Number(pendingId));
    }
}
